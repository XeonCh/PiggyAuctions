...
class AuctionMenu extends Menu
{
    ...
    private function partialRender(): void
    {
        ...
        $this->getInventory()->setItem(33, StringToItemParser::getInstance()->parse("paper")->setCustomName(count($this->auction->getBids()) === 0 ? PiggyAuctions::getInstance()->getMessage("menus.auction-view.no-bids") : PiggyAuctions::getInstance()->getMessage("menus.auction-view.bid-history", ["{BIDS}" => count($this->auction->getBids()), "{HISTORY}" => implode("\n", array_map(static function (AuctionBid $auctionBid): string {
    ...
    public function handle(Item $itemClicked, Item $itemClickedWith, SlotChangeAction $action, InvMenuTransaction $transaction): InvMenuTransactionResult
    {
        switch ($action->getSlot()) {
            case 29:
                ...
                break;
            case 31:
                if ($itemClicked->getTypeId() === ItemTypeIds::GOLD_INGOT) {
                    $this->setInventoryCloseListener(null);
